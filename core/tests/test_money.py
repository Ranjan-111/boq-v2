...
class TestMoney:
    def test_minor_units_exact(self) -> None:
...
class TestPricingKernel:
    ...
    def test_markup_bp(self) -> None:
        base = from_major("1000.00", "INR")
        m = apply_markup(base, 750)  # 7.5%
        assert m == from_major("75.00", "INR")

    def test_markup_compounding_is_explicit_not_implicit(self) -> None:
        base = from_major("1000.00", "INR")
    ...
    def test_money_int_multiply(self) -> None:
