...
| T034 ◐R8 | Scale detection (PROPOSED only): DXF header/INSUNITS, PDF text regex, scale-bar heuristic | P0 | 2 | T032 | never auto-applies. R3: DXF proposal + confirmed-scale validation; R5: PDF `1:N` text regex proposal (PROPOSED-only); R8: proposal wired into parse persistence (BAR_SCALE_DETECTED + exact factor) and the no-units DXF method lie fixed (pushed, CI run 34652270146); scale-bar heuristic later |
...
| T044 ◐R5 | Floor area rules (Gross/Net per room aggregation, storey roll-up) | P0 | 2 | T043 | R5: gross/net per room + floor roll-up done; multi-storey roll-up later. R8 sibling: room.gross.perimeter.v1 (closed-ring perimeter, same kernel refusal gate) |
...
| T047 ◐R8 | PDF vector candidate detectors (areas/lengths/counts + seeded count-by-example) | P1 | 3 | T032 | R5: detectors landed, deliberately NOT registered as rules (candidates propose, humans confirm). R8: area/length candidates surface through the engine as NEEDS_REVIEW measurements (wall-dedup, audited-accept-only, BOQ bills only accepted) (pushed, CI run 34652270146) — count-by-example needs a human seed-picker UI, deferred |
...
| T121 ✅R9 | Golden-run regression suite (determinism) + property-based tests (hypothesis) on geometry/rounding | P0 | 2 | | R9: 25 goldens (every committed fixture, honest refusals included, registry-enforced equality) with version-aware drift verdicts + deterministic regeneration (make golden-update); 37 hypothesis property tests (deterministic profile) on kernel geometry, banker's rounding/pricing, and the replay digest. Third-party parse-refusal message TEXT deliberately not goldened (would make dep upgrades look like engine drift) — refusal + public error type are |
...
| T124 ◐R8 | Security review: authz matrix, upload hardening, rate limits, secrets audit | P0 | 2 | | R8: cross-user authz matrix pinned (scale gate read/confirm, factor validation, project isolation, deleted-project hiding); malformed-id 500s fixed in the last two unguarded resolvers; login pgproto-UUID 500 fixed (pushed, CI run 34652270146). Rate limits + full secrets audit remain |
| T125 ◐R9 | Performance: 50k-entity DXF < 60s parse, viewer < 3s, BOQ 5k recompute < 2s; profiling + indexes | P1 | 3 | | R9: all four measurable §G targets benchmarked green (DXF 50k 1.02s, PDF 100p 0.26s, recompute 5k 0.29s after the N+1 fix was 6.07s, XLSX 5k 0.28s); cProfile recorded; EXPLAIN says existing indexes suffice (no migration) (pushed, CI run 34668532001, perf job green). REMAINS: viewer < 3s first-paint needs a browser harness |
...
| T128 ◐R9 | Deploy: Docker Compose (app/worker/db/minio/caddy) + GH Actions pipeline + prod config + backup | P0 | 3 | | R8: Dockerfile + prod compose (quay.io MinIO) + deploy docs + CI docker-build job + local full-stack smoke test (pushed, CI run 34652270146). R9: GHCR publish (GITHUB_TOKEN, zero new secrets; image verified live at ghcr.io/ranjan-111/boq-v2:main) + caddy TLS profile (ACME, HSTS, fail-fast without domain) + least-privilege MinIO boq-app user (verified live) + worker-unhealthy defect fixed (pushed, CI run 34668532001 incl. the publish job). REMAINS: real server (host + DNS) |
