...
| T128 ◐R9 | Deploy: Docker Compose (app/worker/db/minio/caddy) + GH Actions pipeline + prod config + backup | P0 | 3 | | R8: Dockerfile + prod compose (quay.io MinIO) + deploy docs + CI docker-build job + local full-stack smoke test (pushed, CI run 34652270146). R9: GHCR publish (GITHUB_TOKEN, zero new secrets) + caddy TLS profile (ACME, HSTS, fail-fast without domain) + least-privilege MinIO boq-app user (verified live) + worker-unhealthy defect fixed. REMAINS: real server (host + DNS) |
