...
class TestKeyValidation:
    ...
    def test_rejects_bad_keys(self, mem, bad):
        with pytest.raises(InvalidStorageKey):
            mem.put(bad, io.BytesIO(b"x"), length=1)

    def test_accepts_good_keys(self, mem):
...
class TestLocalStorage:
    ...
    def test_short_write_rejected(self, store):
        with pytest.raises(StorageError):
            store.put("k/a.bin", io.BytesIO(b"hello"), length=3)

    def test_overlong_stream_rejected(self, store):
        with pytest.raises(StorageError):
    ...
    def test_no_partial_files_on_failure(self, store, tmp_path):
        with pytest.raises(StorageError):
