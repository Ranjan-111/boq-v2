...
| T128 ◐R8 | Deploy: Docker Compose (app/worker/db/minio/caddy) + GH Actions pipeline + prod config + backup | P0 | 3 | | R8: Dockerfile + prod compose (quay.io MinIO) + deploy docs + CI docker-build job + local full-stack smoke test (pushed, CI run 34652270146); registry push, TLS, real server remain |
